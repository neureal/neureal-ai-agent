...
class ArchFull(tf.keras.Model):
    def __init__(self, name, inputs, opt_spec, stats_spec, spec_in, spec_out, latent_spec, obs_latent=False, net_blocks=1, net_lstm=False, net_attn=None, num_heads=1, memory_size=None, aug_data_pos=False):
        ...
        for spec in opt_spec:
            self.optimizer[spec['name']] = util.optimizer(name, spec)
        self.stats = OrderedDict()
        for spec in stats_spec:
            self.stats[spec['name']] = {
                'b1':tf.constant(spec['b1'], spec['dtype']),
                'b1_n':tf.constant(1-spec['b1'], spec['dtype']),
                'b2':tf.constant(spec['b2'], spec['dtype']),
                'b2_n':tf.constant(1-spec['b2'], spec['dtype']),
                'dtype':spec['dtype'],
                'ma':tf.Variable(0, dtype=spec['dtype'], trainable=False, name='{}/stats_{}/ma'.format(name, spec['name'])),
                'ema':tf.Variable(0, dtype=spec['dtype'], trainable=False, name='{}/stats_{}/ema'.format(name, spec['name'])),
    ...
    def reset_states(self, use_img=False):
        for layer in self.net.layer_attn:
            layer.reset_states(use_img=use_img)
        for layer in self.net.layer_lstm:
            layer.reset_states()
    ...
    def call(self, inputs, store_memory=True, use_img=False, store_real=False, training=None):
        ...
        for out_n in out:
            ...
            if isinfnan > 0:
                tf.print(self.name, 'net out:', out)
        return out
...
class ArchAR(tf.keras.Model):
    def __init__(self, name, inputs, opt_spec, stats_spec, latent_spec, net_blocks=1, net_lstm=False, net_attn=None, num_heads=1, memory_size=None, mem_img_size=0):
        ...
        self.dist = self.net.dist
        if net_attn['out']:
            self.layer_attn_out = util.MultiHeadAttention(latent_size=latent_size, num_heads=num_heads, norm=False, residual=False, cross_type=1, num_latents=mem_img_size*num_latents, channels=latent_size, name='attn_out')
        else:
            self.layer_dense_out = tf.keras.layers.Dense(mem_img_size*num_latents*latent_size, name='dense_out')
...
class In(tf.keras.layers.Layer):
    def __init__(self, latent_spec, spec_in, obs_latent=False, net_attn_io=False, num_heads=1, aug_data_pos=False):
        ...
        self.net_ins = len(spec_in)
        self.input_names, self.layer_attn_in, self.layer_mlp_in, self.pos_idx_in = OrderedDict(), OrderedDict(), OrderedDict(), OrderedDict()
        for i in range(self.net_ins):
            space_name, input_name, event_size, channels, step_shape, num_latents = spec_in[i]['space_name'], spec_in[i]['name'], spec_in[i]['event_size'], spec_in[i]['channels'], spec_in[i]['step_shape'], spec_in[i]['num_latents']
            self.num_latents += num_latents
            if space_name not in self.input_names:
                self.input_names[space_name], self.layer_attn_in[space_name], self.layer_mlp_in[space_name], self.pos_idx_in[space_name] = 0, [], [], []
            ...
            if aug_data_pos and event_size > 1:
                ...
                self.pos_idx_in[space_name] += [None]
            ...
            #     else: self.layer_attn_in[space_name] += [tf.keras.layers.Dense(num_latents*latent_size, name='dense_in_{}_{}'.format(space_name, input_name))]
                ...
                self.layer_attn_in[space_name] += [None]
        ...
        if obs_latent:
            ...
            latent_spec.update({'num_latents':self.num_latents})
    ...
    def call(self, inputs, training=None):
        ...
        if self.net_attn_io2:
            out = self.layer_attn_io2(out)
        return out
...
class Net(tf.keras.layers.Layer):
    def __init__(self, latent_spec, obs_latent=False, net_blocks=1, net_lstm=False, net_attn=False, net_attn_io=False, net_attn_ar=False, num_heads=1, memory_size=None):
        ...
        for i in range(net_blocks):
            if net_attn:
                ...
                self.layer_mlp += [util.MLPBlock(hidden_size=midp, latent_size=latent_size, evo=evo, residual=False, name='mlp_{:02d}'.format(i))]
        ...
        if obs_latent:
            if net_attn_io:
                self.layer_attn_out = util.MultiHeadAttention(latent_size=latent_size, num_heads=num_heads, norm=False, residual=False, cross_type=1, num_latents=self.num_latents, channels=latent_size, name='attn_out')
            else:
                self.layer_dense_out = tf.keras.layers.Dense(self.num_latents*latent_size, name='dense_out')
        if self.dist_out:
            self.layer_out_logits = util.MLPBlock(hidden_size=outp, latent_size=params_size, evo=evo, residual=False, name='mlp_out_logits')
    ...
    def call(self, inputs, store_memory=True, use_img=False, store_real=False, training=None):
        ...
        for i in range(self.net_blocks):
            if self.net_attn:
                out = tf.squeeze(self.layer_attn[i](tf.expand_dims(out, axis=0), auto_mask=training, store_memory=store_memory, use_img=use_img, store_real=store_real), axis=0)
            if self.net_lstm:
                out = tf.squeeze(self.layer_lstm[i](tf.expand_dims(out, axis=0), training=training), axis=0)
            out = self.layer_mlp[i](out)
        if self.obs_latent:
            if self.net_attn_io:
                out = self.layer_attn_out(out)
            else:
                out = tf.reshape(self.layer_dense_out(tf.reshape(out, (1, -1))), (self.num_latents, -1))
        if self.dist_out:
            out = self.layer_out_logits(out)
        return out
...
class Out(tf.keras.layers.Layer):
    def __init__(self, latent_spec, spec_out, net_attn_io=False, num_heads=1):
        ...
        for i in range(self.net_outs):
            ...
            if net_attn_io:
                self.layer_attn_out[i] = util.MultiHeadAttention(latent_size=latent_size, num_heads=num_heads, norm=False, residual=False, cross_type=1, num_latents=max_batch_out*event_size, channels=latent_size, name='attn_out_{}_{}'.format(space_name, output_name))
            else:
                self.layer_dense_out[i] = tf.keras.layers.Dense(event_size*latent_size, name='dense_out_{}_{}'.format(space_name, output_name))
    ...
    def call(self, inputs, batch_size=tf.constant(1), training=None):
        ...
        for i in range(self.net_outs):
            if self.net_attn_io:
                out_logits[i] = self.layer_attn_out[i](out) # out_logits[i] = self.layer_attn_out[i](out, num_latents=batch_size*self.event_size[i])
            else:
